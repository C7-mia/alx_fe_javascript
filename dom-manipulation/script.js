...
// Add new quotes from server if not already present
async function syncWithServer() {
  const serverQuotes = await fetchQuotesFromServer();

  const newQuotes = serverQuotes.filter(
    (serverQ) => !quotes.some((localQ) => localQ.id === serverQ.id)
  );
...
// Send new quotes to server (simulated POST)
...
function setup() {
  ...
  restoreLastQuote();
  syncQuotes(); // initial
  setInterval(syncQuotes, syncInterval); // every 60s
}
